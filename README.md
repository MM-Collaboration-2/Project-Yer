...
1 + "str"   ' = 0'
```
6. Так как архитектурно для вычисления выражения оно приводится к постфиксному виду, то интерпретатор способен вычислять и выражения, уже приведенные к постфиксному виду. **НАСТОЯТЕЛЬНО НЕ РЕКОМЕНДУЕТСЯ ТАК ДЕЛАТЬ!**. Выражения такого вида могут привести к неопределенному поведению.
```
1 2 +       ' = 3'
```
